...
- [ ] 来源列表筛选、分页正常。
- [ ] 来源目录默认只常驻搜索；类型、运行状态、删除状态与排序收进“筛选与排序” Sheet，开关 Sheet 不挤压列表且窄屏无横向溢出。
- [ ] 列表可逐项勾选或选择最多 200 个“当前筛选全部”，超过上限时明确提示缩小筛选；已删除来源不可选择。
- [ ] 批量动作只在进入“批量选择”后出现；退出批量选择会清除当前选择，默认目录不常驻大块批量工具栏。
- [ ] 列表能区分最近成功同步，并展示未提交/排队/处理中/失败下载数、任务状态和下次执行。
...
- [ ] 点击来源后 URL 包含 `sourceId`；刷新仍停留在同一来源，返回“来源目录”会清除该参数并恢复列表。
- [ ] 来源详情不使用窄侧栏，按“概览 / 发现的 Tweet / 扫描历史 / 更多设置”分层；扫描与下载控制在概览，Tweet 筛选收进 Sheet，提交导入、游标信息和删除在更多设置。
- [ ] 来源详情中最近扫描记录、发现数量、新增数量、重复数量、媒体预估数量可读。

### Search, Library, Failures, Duplicates

- [ ] 系统概览、归档队列、失败工作台、重复媒体与系统操作在桌面和窄屏下均无页面级横向溢出；页头、操作密度和分区层级保持一致。
- [ ] 归档队列不重复显示全局“实时事件已连接”文案，批次状态与连接状态语义分离。
...
- [ ] Feed 卡片最多展示三个标签、合集数量和私人备注标记；快捷整理可保存标签、合集与纯文本备注。
- [ ] Tweet Detail 使用连续内容流，返回、归档状态、媒体、整理信息和尝试记录在桌面与窄屏下均可访问；标签、合集与备注仅在点击“保存整理”后一次性落库。
- [ ] 整理弹窗存在未保存更改时，取消、关闭按钮、遮罩或 Escape 均先要求确认放弃；保存失败后输入仍保留且可以重试。
- [ ] “合集”页面可创建、编辑和浏览合集，查看成员 Tweet，并从合集现有媒体中选择封面；独立的“自定义标签”页面支持搜索、排序以及名称、颜色和描述维护。
- [ ] 删除标签或合集会展示影响的 Tweet 数并再次确认；确认后只解除关系，Tweet、媒体、任务历史和私人备注仍保留。
- [ ] Library 默认“整理 Tweet”模式按 Tweet 去重选择，最多 200 条批量加减标签/合集；切换到“删除媒体”后才按 `media_assets.id` 选择。
...
- [ ] “归档洞察”可从侧边栏与命令面板进入；桌面和窄屏均无横向页面溢出。
- [ ] 发布时间趋势只使用 `published_at`，入库趋势只使用 `imported_at`，两者文案和含义明确区分。
- [ ] 媒体类型、媒体状态、作者空间占用、整理覆盖率和元数据完整率与数据库抽查结果一致。
- [ ] 来源发现状态按 Tweet 去重，且明确说明发现、来源提交和当前校验不是严格线性漏斗。
- [ ] 页面请求仅调用只读 `GET /api/v1/library/insights`；加载页面不会触发 verify、磁盘扫描、下载或任何 API 写请求。
- [ ] 空库显示空态，API 失败显示可重试错误态；调试模式会遮蔽作者身份。
