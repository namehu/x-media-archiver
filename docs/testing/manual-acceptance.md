...
- [ ] Feed 的来源、作者、正文、媒体类型、全部/喜欢范围、最新/随机排列和随机 seed 同步到 URL；刷新或复制 URL 后恢复同一筛选与随机顺序。
- [ ] 随机时间线在同一 seed 下跨页不重复；切换全部/喜欢或应用筛选不更换 seed，只有“重新随机”生成新 seed并回到顶部；本轮结束后不自动循环。
- [ ] Feed 全屏预览可上下切换 Tweet、左右切换同一 Tweet 的媒体；触摸、滚轮/触控板、上下方向键、J/K 和可见按钮均可操作，视频横向快进与控制区不会误触发 Tweet 切换。
- [ ] 预览接近已加载末尾时自动加载下一页；加载失败保留当前 Tweet 并可重试，首尾不循环；Back、Escape 或关闭按钮一次退出并恢复 Feed 滚动位置和视频进度。
...
- [ ] 空库显示空态，API 失败显示可重试错误态；媒体隐私模式会遮蔽作者身份。
...
- [ ] 在窄屏检查长 Hashtag 不撑破卡片或预览；开启媒体隐私模式后平台 Hashtag 被遮罩，站内搜索链接仍可用。

### 成人内容 Gate 与媒体隐私模式

- [ ] 账号未开启媒体隐私模式且当前标签页未确认时，直接访问任意业务路由只显示成人内容提示；业务路由和归档媒体均未请求。
- [ ] 确认年满 18 岁后，当前标签页刷新和站内跳转不再提示；新标签页仍需确认，退出并重新登录后当前标签页也会再次提示。
- [ ] 选择“开启隐私模式后继续”会持久化账号偏好，重新登录后直接进入隐藏状态。
- [ ] 隐私模式下可进入 Tweet 详情和其他站内页面；图片、视频与播放器均不挂载，显示等比例“媒体已隐藏”占位，X/Twitter 外链不可用。
- [ ] 关闭隐私模式时，当前标签页未确认则必须先确认成人内容；只有账号偏好保存成功后才写入标签页确认并显示媒体，取消或保存失败时媒体不能提前显示。
- [ ] 浅色、深色和 390px 窄屏下 Gate 无横向溢出，主操作默认获得键盘焦点，错误后可重试。
