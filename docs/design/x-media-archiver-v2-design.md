...
- [x-media-archiver-final-design.md](./x-media-archiver-final-design.md)：V0/V1 基础架构与归档链路设计基线。
- [roadmap-todo.md](./roadmap-todo.md)：当前已实现能力与现有待办清单。
...
说明：现有 `roadmap-todo.md` 中与可靠性相关的 V1 待办仍保持原样；本文定义的是进入本地控制台前必须具备的产品能力，不变更既有完成状态。
...
本文档用于固定产品方向和后续开发边界。本次文档归档不实施以下内容：
