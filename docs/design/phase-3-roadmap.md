...
3. 可观测性增强
   ...
   - 后续如需继续增强，再把 downloader/source worker 的业务事件逐步补充为带 extra 字段的结构化日志。
4. E2E / 手工验收固化
...
1. **后端 API 层** — 路由已拆到 `cli/xarchiver/api/v1/`，`app.py` 已瘦身；response model、健康详情和结构化日志基础已完成；下一步缺口是更完整的服务层测试和 worker 业务日志细化。
2. **WebUI** — 分页、SSE 状态和 OpenAPI 类型同步基础已接入；下一步缺口是 Dialog/Toast/DataTable/Skeleton/EmptyState 等长期组件，以及 AppShell 信息架构。
3. **工程基建** — GitHub Actions、测试隔离和 OpenAPI 类型生成已接入；下一步缺口是 lint/pre-commit、服务层测试补齐和更细的 worker 业务日志。
...
**结论**：框架不需要推倒，P3 已完成主要边界设施建设。后续重点转为服务层测试、worker 业务日志细化和 WebUI 长期体验硬化；E2E 暂不作为近期任务。
...
- service 中高价值业务事件后续逐步补充结构化 extra 字段
- `/api/v1/health/detail` 已返回：worker 写锁、队列积压、来源扫描状态与最近错误摘要

### 3.6 测试补齐（贯穿 P3）

补齐 [cli/tests/](../../cli/tests/) 中 `recovery.py` / `archive.py` 等服务层单测；API 路由层已有冒烟测试，SSE 仍以现有事件总线单测和手工验收为主。
...
| **P3.4** | 可观测性 | ✅ `/api/v1/health/detail`、WebUI 顶栏健康状态、Operations 系统状态面板、最近错误定位跳转、健康响应模型与定位测试、API JSON 结构化日志/request id；⏳ worker 业务日志细化、实时日志尾巴、错误分类视图 | 仅追加，不修改既有行为 |
