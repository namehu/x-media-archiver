...
1. `response_model` 收口
   ...
   - 后续继续把宽模型逐步收紧到明确字段，尤其是详情页中的 `dict[str, Any]` 嵌套数据。
2. API client 进一步分层
...
5. WebUI 长期体验
   - AppShell、Toast/Dialog/Skeleton/EmptyState/ErrorState、危险动作输入确认、主题 token。
   - i18n 中英文补齐继续保持为硬约束。
...
- ⏳ Archive Queue 主流程 E2E 仍可作为后续专门验收项，不阻塞本次 P3.6 代码收口。
...
1. **后端 API 层** — 路由已拆到 `cli/xarchiver/api/v1/`，`app.py` 已瘦身；下一步缺口是 response model、健康详情、结构化日志和更完整的服务层测试。
2. **WebUI** — 分页、SSE 状态和 OpenAPI 类型同步基础已接入；下一步缺口是 Dialog/Toast/DataTable/Skeleton/EmptyState 等长期组件，以及 AppShell 信息架构。
3. **工程基建** — GitHub Actions、测试隔离和 OpenAPI 类型生成已接入；下一步缺口是 E2E、lint/pre-commit 和更严格的 schema 响应模型。
...
**结论**：框架不需要推倒，P3 已完成主要边界设施建设。后续重点转为 response model、可观测性、E2E 和 WebUI 体验硬化。
...
| **P3.4** | 可观测性 | ✅ `/api/v1/health/detail`、WebUI 顶栏健康状态、Operations 系统状态面板、最近错误定位跳转、健康响应模型与定位测试；⏳ 结构化日志、实时日志尾巴、错误分类视图 | 仅追加，不修改既有行为 |
