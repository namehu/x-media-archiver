...
> 状态：已启动，P3 第一阶段、第二阶段、第三阶段、第四阶段实时刷新基础已完成
> 评审日期：2026-05-27
...
- ✅ `/api/archive-runs`、`/api/sources`、`/api/duplicates` 支持分页与筛选，并沿用 `PageResponse` 响应形态。
...
- ✅ 引入 `openapi-typescript`，新增 `webui/src/api/openapi.json` 与 `webui/src/api/generated.ts`。
...
- ⏳ 复杂 `response_model` 暂不补，等分页接口和响应形态继续稳定后再做。
...
- ✅ 新增 `GET /api/events` SSE endpoint，支持 `topics` 过滤，当前 topic 包含 `archive_runs`、`sources`、`source_scans`。
...
- ⏳ `/api/health/detail`、结构化日志、worker 锁状态事件仍留在后续可观测性批次。
...
| **P3.3** | 实时性 | ✅ `core/events.py` 事件总线、`/api/events` SSE、WebUI `useServerEvents`、事件连接状态指示；⏳ worker 锁状态事件后续补 | SSE 只负责刷新；失联降级到 15s 条件轮询，不能丢失历史诊断能力 |
