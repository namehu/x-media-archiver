...
| 组件库 | **渐进引入 shadcn/ui** | 新建 `webui/src/components/ui-next/`,旧 `components/ui/` 保留,M2/M3 重构按页整页切换。 |
...
### 3.2 组件路线 — ui-next 共存,整页切换

- 新建 `webui/src/components/ui-next/`,从 shadcn/ui CLI 复制 17 个组件源码进来 (Button/Card/Input/Select/Badge/Dialog/Sheet/Tabs/Tooltip/Toast(sonner)/Command/Table/Skeleton/Popover/DropdownMenu/Checkbox/Switch),统一接 design system token
...
- Dashboard 页面用 ui-next + 新 token 完成首发改版
...
4. ☐ `webui/demo` 路由能预览全部 ui-next 组件(类 Storybook)
...
| 风险 | 应对 |
|---|---|
| shadcn 接入后与现有自建组件混搭丑陋期 | `ui-next/` 隔离;每 PR 只动一页,可独立 revert |
...
- **API v1 响应模型收口** → ui-next 的 DataTable 列定义直接绑 generated.ts 类型
...
| [webui-component-inventory.md](./webui-component-inventory.md) | ui-next 组件清单 + 旧→新迁移映射 + 8 页面组件使用指南 | 前端实施 / 重构期对照表 |
