...
8. Sources 已支持持久化人工批量任务、逐来源结果、10 来源波次、失败项重试，以及刷新后精确下载本轮新增。
9. 来源命名定时策略、错过/重叠合并、下载上限与扫描/下载公平网络调度已实现；策略默认关闭。

保持不做：

1. 不默认执行真实 X 大批量扫描或下载。
2. 普通来源扫描不隐式提交下载；只有用户明确选择的“更新并下载新增”任务或已启用的同类定时策略会提交本轮新增。
3. 不做媒体文件物理删除。
...
- [ ] 验证扫描与下载同时就绪时由单网络 worker 交替执行，不并发启动外部下载器且两类工作都不会饥饿。
...
- [x] 实现历史扫描之外的命名“定期最新补扫”策略，支持间隔、每日和每周锚点。
- [x] 明确补扫频率、错过/重叠合并、暂停入口、下载上限、认证熔断和失败恢复语义。
