...
export function FailuresPage() {
  ...
  if (isLoading) return <State text="Loading failures" />;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        {data?.rows.length === 0 ? <p className="text-sm text-muted-foreground">No failures found.</p> : null}
        {data?.rows.map((row) => (
          <div key={row.tweet_id} className="rounded-md border border-border p-3">
            ...
            <div className="mt-2 grid gap-2 text-sm text-muted-foreground md:grid-cols-3">
              <span>Engine: {row.latest_engine || "-"}</span>
              <span>Retries: {row.retry_count ?? 0}</span>
              <span>Finished: {formatDateTime(row.latest_finished_at)}</span>
            </div>
          </div>
        ))}
      </CardContent>
