...
type ChartPayloadItem = {
  ...
  type?: string;
  value?: { toLocaleString: () => string } | string | number;
};
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "grid min-w-32 items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-3",
          className,
        )}
      >
    ...
    >
      {payload
        ...
        .map((item) => {
          ...
          return (
          <div key={String(item.value ?? item.dataKey)} className="flex items-center gap-1.5 [&>svg]:size-3 [&>svg]:text-muted-foreground">
