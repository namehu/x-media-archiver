...
def scan_source(source_id: int, limit: int = 20) -> dict[str, object]:
    ...
    records, scan_meta = discover_records_with_gallery_dl(scan_url, limit)
    if not records:
        mark_source_scan_result(
            source_id,
            error_category=scan_meta.get("error_category") or "download_no_output",
            error_message=scan_meta.get("error_message") or "No tweets discovered for source.",
        )
        return {
            "source_id": source_id,
            "discovered_count": 0,
...
def discover_records_with_gallery_dl(source_url: str, limit: int) -> tuple[list[dict[str, Any]], dict[str, object]]:
    if limit < 1:
...
def parse_gallery_dl_records(stdout: str, source_url: str) -> list[dict[str, Any]]:
    ...
    for event in payload if isinstance(payload, list) else []:
        ...
        if tweet_id in rows:
            rows[tweet_id] = {
                **next_row,
                **{key: value for key, value in rows[tweet_id].items() if value not in (None, "")},
                "raw_import": metadata,
            }
        else:
...
def record_source_discoveries(
    ...
    with connect() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                set discovered_count = (
                    ...
                    newest_seen_tweet_id = coalesce(newest_seen_tweet_id, %s),
                    oldest_seen_tweet_id = %s,
        ...
        conn.commit()
    return {"discovered_count": len(unique_tweet_ids), "new_discovered_count": inserted}
